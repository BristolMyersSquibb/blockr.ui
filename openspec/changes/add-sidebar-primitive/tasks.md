...
- [x] 2.4 Implement `R/sidebar.R`:
  ...
  - `confirm_sidebar(id, ui, title = NULL, session = shiny::getDefaultReactiveDomain())` — convenience helper that branches on `sidebar_state(id, session)$pinned`: pinned -> `show_sidebar(id, ui, title, session)` (re-render fresh body for chaining), unpinned -> `hide_sidebar(id, session)`. Lives in `blockr.ui` so consumers (e.g. `blockr.dock`) don't duplicate the chain/hide logic.
