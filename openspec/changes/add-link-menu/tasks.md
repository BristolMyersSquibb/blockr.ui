...
- [ ] 1.1 `inst/assets/css/blockr-link-menu.css`:
...
- [ ] 1.2 `inst/assets/js/blockr-link-menu.js`:
  ...
  - `receiveMessage` handles `{ type: "pool-update", eligible: { outgoing, incoming }, link_id_seed }`: toggle `.hidden` on every card against `eligible[data-direction]`, recompute the panel's `is-empty` class, optionally re-seed per-card Link ID inputs from `link_id_seed` (skip fields the user has edited; append a per-card monotonic suffix so the seed produces distinct ids across cards). MUST NOT touch card-expansion state, scroll position, or the search input value.
- [ ] 1.3 `R/link-menu.R` (Shiny module):
  ...
  - `link_eligible_pools(board, anchor)` (exported): returns `list(outgoing = <chr>, incoming = <chr>)`. Internally split into two small helpers `link_outgoing_targets(board, anchor)` (non-anchor blocks with at least one free input or variadic) and `link_incoming_sources(board, anchor)` (returns `character()` when anchor has no free input and isn't variadic; otherwise every non-anchor block). Same `anchor` validation as `link_menu_ui()`.
...
- [ ] 1.4 Unit tests `tests/testthat/test-link-menu.R`:
  ...
  - `link_eligible_pools(board, anchor)`: returns both pools for a variadic anchor; returns `incoming = character()` when the anchor has no free input and isn't variadic; raises `blockr_ui_link_menu_unknown_anchor` for an unknown anchor id.
  - Malformed inputs rejected.
- [ ] 1.5 shinytest2 `tests/testthat/test-link-menu-shinytest2.R` against `inst/examples/link-menu/app.R`:
...
- [ ] 1.6 Runnable example `inst/examples/link-menu/app.R` exercising the flow against a small board with a mix of arity-1, arity > 1, and variadic blocks. Provide buttons that open the menu anchored on a source-only block, a target-only block, and a variadic block so the suite hits all three layout cases.
- [ ] 1.7 Vignette `vignettes/link-menu.Rmd`: minimal example + commit-spec shape + the bidirectional behaviour.
- [ ] 1.8 README screenshot: a third figure under "Example", captured via shinytest2 in `README.Rmd`, named `man/figures/link-menu.png` (anchored on a block whose OUTGOING section is non-trivial so the picker is interesting).
- [ ] 1.9 `devtools::check()` clean (0/0/1 timestamp NOTE); `lintr::lint_package()` clean under the CI config (`linters_with_defaults(object_name_linter = NULL)`); `openspec validate add-link-menu --strict` clean.
