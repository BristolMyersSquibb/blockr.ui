...
To inspect which plugin are by default used in the app, you can call the `blockr.core::board_plugins()` generic, like so:

```{r}
board_plugins(my_board)
```

The set of plugins can be overridden in calls to `serve()`.
