...
```{r link-menu-example, eval=FALSE, code=readLines("inst/examples/link-menu/app.R")}
```

```{r link-menu-screenshot, echo=FALSE, eval=TRUE, message=FALSE, warning=FALSE, results='hide'}
withr::local_envvar(NOT_CRAN = "true")
app <- shinytest2::AppDriver$new(
  system.file("examples", "link-menu", package = "blockr.ui"),
  width = 1280,
  height = 820
)
Sys.sleep(2)
# Anchor m: shows both OUTGOING and INCOMING sections (most
# interesting layout case).
app$run_js("document.getElementById('open_m').click()")
Sys.sleep(1)
tmp <- tempfile(fileext = ".png")
app$get_screenshot(tmp)
file.copy(tmp, "man/figures/link-menu.png", overwrite = TRUE)
app$stop()
```

![blockr.ui link menu example](man/figures/link-menu.png)
